                ...
                <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200/60 dark:border-slate-800/60 overflow-hidden stagger-2 animate-slide-up">
                    ...
                    <div class="p-8 space-y-6">
                        <div class="flex items-center gap-3 pb-4 border-b border-slate-100 dark:border-slate-800">
                            ...
                            <h3 class="text-lg font-bold">Admin Profile</h3>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="form-group group">
                                <label class="form-label">Display Name</label>
                                <input type="text" name="admin_name" class="form-input" value="{{ $setting->admin_name }}" required>
                            </div>
                            <div class="form-group group">
                                <label class="form-label">Email Address</label>
                                <input type="email" name="admin_email" class="form-input" value="{{ $setting->admin_email }}" required>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-8 stagger-3 animate-slide-up">
                <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200/60 dark:border-slate-800/60 overflow-hidden">
                    <div class="p-8 text-center space-y-6">
                        <label class="form-label block text-center mb-4">Admin Photo</label>
                        <div class="relative inline-block group">
                            <div class="w-32 h-32 rounded-3xl overflow-hidden border-4 border-white dark:border-slate-800 shadow-2xl mx-auto ring-4 ring-orange-500/20">
                                @if($setting->admin_photo)
                                    <img src="{{ asset('storage/' . $setting->admin_photo) }}" class="w-full h-full object-cover" id="adminPhotoPreview">
                                @else
                                    <img src="https://api.dicebear.com/7.x/avataaars/svg?seed={{ $setting->admin_name }}" class="w-full h-full object-cover" id="adminPhotoPreview">
                                @endif
                            </div>
                            <label class="absolute -bottom-2 -right-2 w-10 h-10 bg-orange-500 text-white rounded-xl flex items-center justify-center shadow-lg cursor-pointer hover:scale-110 transition-transform border-4 border-white dark:border-slate-900">
                                ...
                                <input type="file" name="admin_photo" class="hidden" onchange="previewImage(this, 'adminPhotoPreview')">
                            </label>
                        </div>
                        
                        <div class="pt-4">
                            <h4 class="font-extrabold text-xl">{{ $setting->admin_name }}</h4>
                            <p class="text-[10px] text-orange-500 font-bold uppercase tracking-widest mt-1">Super Administrator</p>
                        </div>
                ...
                <div class="p-6 bg-orange-500/5 rounded-2xl border border-orange-500/10">
                    <p class="text-xs text-orange-600 dark:text-orange-400 leading-relaxed font-medium">
                        <i class="fas fa-info-circle mr-1"></i> These settings control the global identity of your system, including the sidebar branding and administrator contact info.
                    </p>
                </div>
            </div>
        </div>
